...
def calculate_demographic_data(print_data=True):
  ...
  percentage_bachelors = round(
      df['education'].value_counts(normalize=True)['Bachelors'] * 100.0, 1)
  ...
  higher_education_rich = round(
      100.0 * (higher_education['salary'] == '>50K').sum() /
      len(higher_education), 1)
  lower_education_rich = round(
      100.0 * (lower_education['salary'] == '>50K').sum() /
  ...
  rich_percentage = round(
      100.0 * (num_min_workers['salary'] == '>50K').sum() /
  ...
  highest_earning_country_percentage = round(
      100 * highest_earning_country_percentage, 1)
